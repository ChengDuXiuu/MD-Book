...
```shell
-u ：去除重复行,不管是否重复行连续，只要重复则去除，uniq只取出连续的重复行
-r ：降序排列，默认是升序
...
> tee工具是从标准输入读取并写入到标准输出和文件，即：双向覆盖重定向（屏幕输出|文本输入），即同时输出到屏幕并打印到文件中

~~~shell
选项：
-a 双向追加重定向，默认是覆盖

# echo hello world  #只向屏幕输出内容
# echo hello world|tee file1  #将echo内容同时向屏幕和文件输出
# cat file1 
...
![image-20220320215332347](/image-20220320215332347-7784417.png)
...
```shell
用法1：命令的执行结果交给tr处理，其中string1用于查询，string2用于转换处理，即将commands命令的结果搜索string1并替换为string2

 commands|tr  'string1'  'string2'

用法2：tr处理的内容来自文件，记住要使用"<"标准输入，将文件中string1替换为string2

 tr  'string1'  'string2' < filename

用法3：匹配string1进行相应操作，如删除操作，搜索文件中string1的字符然后进行options操作
...
| [:punct:]          | 匹配所有标点符号     |                                                              |
| [:space:]          | 所有水平或垂直的空格 |                                                              |
| [:cntrl:]          | 所有控制字符         | \f Ctrl-L        走行换页<br/>\n Ctrl-J  	换行<br />\r Ctrl-M     回车<br />\t Ctrl-l       tab键<br /> |
...
# rm -f file*
# cp *.conf  /dir1
# touch file{1..5}   和[1-5]一样
# ls file[1-5].txt   列出file1.txt、file2.txt。。。file5.txt
# ls file[1,5].txt   列出file1.txt、file5.txt
~~~
