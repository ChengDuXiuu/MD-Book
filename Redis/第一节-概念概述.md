...
8. **Redis工作线程无论什么版本只有一个，6.X之后io多线程**
...
* 数据库关系型有很多分组、limit、join操作，而Redis没有集合操作

* 单线程+io多路复用 

	* 多线程+锁处理逻辑 ：每个线程去不断地刺探请求是否准备好，准备好后交由工作线程去处理请求，期间轮询请求状态的时候，工作线程处于就绪状态。

	* 单线程+io多路复用处理逻辑 ： 监视所有请求，当请求准备完毕后，工作线程（只有一个工作线程即单线程由来）直接去处理请求，即工作线程一直在执行，没有空档，
...
![image-20210828213215414](第一节-概念概述.assets/image-20210828213215414.png)
...
#是否禁止复制tcp链接的tcp 
#nodelay参数，可传递yes或者no。默认是no，即使用tcp nodelay。
