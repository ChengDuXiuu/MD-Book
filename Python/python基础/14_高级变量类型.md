...
* Python3 新增数据类型，是Byte的序列。字符串是字符（Unicode）的序列。
* bytes 只是简单地记录内存中的原始数据，至于这些数据到底表示什么内容（字符串、数字、图片、音频等），完全由程序的解析方式决定。非常适合在互联网上传输
...
![image-20220703122748445](images/image-20220703122748445.png)
